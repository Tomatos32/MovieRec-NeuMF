...
<template>
  ...
  >
    <nav
      ref="navRef"
      :class="[
        'card-nav block h-[60px] p-0 rounded-xl shadow-md relative overflow-hidden will-change-[height] backdrop-blur-md backdrop-saturate-150',
        { open: isExpanded }
      ]"
        ...
        >
          ...
          <div class="flex flex-col gap-[2px] mt-auto nav-card-links">
            <a
              ...
              :key="`${lnk.label}-${i}`"
              :class="[
                'inline-flex items-center gap-[6px] hover:opacity-100 text-[15px] md:text-[16px] no-underline transition-all duration-300 cursor-pointer nav-card-link',
                route.path === lnk.href ? 'font-bold opacity-100 scale-[1.02] origin-left' : 'font-normal opacity-75'
              ]"
              @click.prevent="handleNavigation(lnk.href)"
              :href="lnk.href"
              :aria-label="lnk.ariaLabel"
            >
              <ArrowUpRight class="nav-card-link-icon shrink-0 w-4 h-4" aria-hidden="true" />
              {{ lnk.label }}
            </a>
          </div>
        </div>
